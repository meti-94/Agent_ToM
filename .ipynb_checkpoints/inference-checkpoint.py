...
def main(args):
    sae_model=None
    tokenizer, model = load_vLLM_model(args.model_name, device=args.device, seed=args.seed, tensor_parallel_size=args.num_gpu)
    ...
    for index in tqdm(range(args.begin, args.end)):
        ...
        if args.search_algo=='sae':
            ...
            if sae_model==None:
                ...
                model2path = {
                    'google/gemma-2-2b':'/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/llms/gemma-2-2b/',
                    'google/gemma-2-9b':'/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/llms/gemma-2-9b/',
                }
                hf_tokenizer = AutoTokenizer.from_pretrained(model2path[args.sae_base_model])
                hf_model = AutoModelForCausalLM.from_pretrained(model2path[args.sae_base_model])
                sae_model = HookedTransformer.from_pretrained(model_name = args.sae_base_model, 
                                                              hf_model=hf_model, 
                ...
                sae2path = {
                    'gemma-scope-2b-pt-res-canonical': '/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/saes/gemma-2-2b-25/', 
                    'gemma-scope-9b-pt-res-canonical': '/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/saes/gemma-2-9b-38/', 
                }
                sae = SAE.load_from_disk(sae2path[args.release], device = args.sae_device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="cuda:0", help="Device for vllm: 'cpu', 'cuda:0', 'mps', etc.")
    parser.add_argument("--sae_device", type=str, default="cuda:1", help="Device for sae: 'cpu', 'cuda:0', 'mps', etc.")
    ...
    parser.add_argument("--cache_file", type=str, default="./cache/indices.json", help="Path to file where associated indices were saved.")
    parser.add_argument("--steering_off", action="store_true",help='This is a boolean parameter who turns steering hook deactivate if it is True')
    parser.add_argument("--generation_temp", type=float, default=0.1, help='This parameter sets the temperature for thought generation.')
